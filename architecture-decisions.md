...
| Daily bucketing (`buildDailyCounts`) | Seeds a `Map` with one UTC-day key (`toISOString().slice(0, 10)`) per day from `since` (30 days before the request) through "now", then increments the matching key per event; returns exactly 31 entries | `since` and the loop's `now` are both real-time `Date` instants captured seconds apart (a DB round trip), never a full day apart, so the day count is always exactly 31 regardless of local timezone or time of day; using UTC methods throughout keeps day keys independent of server locale |
