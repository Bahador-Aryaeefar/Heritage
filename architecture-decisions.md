...
| FA/EN/AR content editor UX | Admin `SiteForm` renders `Tabs` from `CONTENT_LOCALE_DEFINITIONS` (`fa` \| `en` \| `ar`) each holding title, shortDescription, and a `BlockListEditor`; EN and AR have "Copy from Persian" (`copyBlocksFromFa()`) with confirm when target blocks exist. **Superseded 2026-07-21 (see §21):** editor now requires all three locales, rich spans, DnD reorder, and per-tab undo | Matches the public page's fa/en/ar model 1:1; avoids re-uploading the same photo/audio per locale |
